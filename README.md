...
* [`src/cljs/fi/velotoken/ux/`](src/cljs/fi/velotoken/ux/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/fi/velotoken/ux/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `fi.velotoken.ux` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/fi/velotoken/ux/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns fi.velotoken.ux.example
  (:require [fi.velotoken.ux.config :as config])
